...
function parser(json) {
  if (!json) {
    var contents = fs.readFileSync('roadmap_061517.json');
  ...
  u.forEach(json.cards, card => {
    // ignore all cards without a label (too old to care about)
    var cardLabels = u.map(card.labels, l => l.name).join(', ');
    ...
    var cardActions = u.filter(actions, a => a.data && a.data.card && a.data.card.id === card.id);

    // what list is this card in right now? 
    var cardList = u.find(lists, l => l.id === card.idList).name;

    // when did we start the project? (actions are an array in reverse chronological order)
    var inProgressAction = u.find(actions, a => a.data && a.data.card && a.data.card.id === card.id && a.data.listAfter && a.data.listAfter.name === 'In Progress');
    ...
    if (card.dueComplete) {
        ...
        completeDate = u.find(cardActions, a => a.type === 'updateCard' && a.data.listAfter && a.data.listAfter.name == 'Done');
    }
    var completedDate = u.find(cardActions, a => a.type === 'updateCard' && a.data.listAfter && a.data.listAfter.name == 'Done');
    completedDate = completedDate ? parseDate(completedDate.date) : null;

    // did the due date for this card change? 
    var dueDateChanged = u.filter(cardActions, a => a.data.card && a.data.card.id === card.id && a.data.old && a.data.old.due);
    var oldDueDates = u.map(dueDateChanged, d => d && d.data && d.data.old && parseDate(d.data.old.due)).join(', ');

    // who worked on this card?
    var cardMembers = u.map(card.idMembers, i => u.find(members, m => m.id === i).fullName);

    // do we have a design attached? 
    var design = u.filter(card.attachments, a => a.url.indexOf("docs.google.com") > -1).length;

    // do we have a github attached?
    var github = u.filter(card.attachments, a => a.url.indexOf("github.com") > -1).length;
    ...
    if (customFieldValues) {
      ...
      // var ricePluginInfo = JSON.parse(u.find(json.pluginData, p => p.idPlugin === '56d5e249a98895a9797bebb9').values);
      // { fields:
  ...
  // u.forEach(json.actions, action => {

  //   if (action.type === 'updateCard' && action.data.listAfter && action.data.listAfter.name == 'Done') {
  //     var card = u.find(cards, c => c.id === action.data.card.id);

  //     // did the due date for this card change? 
  //     var dueDateChanged = u.filter(json.actions, a => a.data.card && a.data.card.id === card.id && a.data && a.data.old && a.data.old.due);
  //     oldDueDates = u.map(dueDateChanged, d => parseDate(d.data.old.due));

  //     //look up the memebers
  //     var idMembers = u.map(card.idMembers, i => u.find(members, m => m.id === i).fullName);

  //     //get labels
  //     var labels = u.map(card.labels, l => l.name);

  //     //get start date - when did this move to in progress
  //     var inProgressAction = u.find(json.actions, a => a.data && a.data.card && a.data.card.id === card.id && a.data.listAfter && a.data.listAfter.name === 'In Progress');
  //     inProgressDate = inProgressAction ? parseDate(inProgressAction.date) : null;

  //     // get design docs and github
  //     var attachments = card.attachments;
  //     var design = u.filter(attachments, a => a.url.indexOf("docs.google.com") > -1);
  //     var github = u.filter(attachments, a => a.url.indexOf("github.com") > -1);
